...
class GetCurrentWeatherUseCaseImpl(private val weatherRepository: WeatherRepository) : GetCurrentWeatherUseCase {
    ...
    override fun invoke(city: String, unit: TemperatureUnits): Flow<Resource<CurrentWeather>> = flow {
        try {
            emit(Resource.Loading())
            val weather = weatherRepository.getCurrentWeather(city, unit.networkParam).toCurrentWeather(unit)
